...
def  AccountSetting(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.add_message(request, messages.INFO, 'store_name is already in use. Please pick another store name.')
    else:
...
def StoreOrders(request):
    ...
    orderItems = OrderItem.objects.filter(product__vendor = request.user).order_by('-date_add')
    orders =[]
...
def SotreOrdersDetail(request, pk):
    ...
    orderItems = OrderItem.objects.filter(product__vendor = request.user, order=order)
